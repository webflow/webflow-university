...
(() => {
  ...
  function parseEventData(element) {
    ...
    const occurrences = datetimeFlatlist.split(",").map((dateString) => dateString.trim()).filter(Boolean).map((dateString) => {
  ...
  function findNextOccurrence(eventData) {
    ...
    const nextOccurrence = eventData.occurrences.find(
      (occurrence) => occurrence.toMillis() > now2.toMillis()
    );
    if (!nextOccurrence) {
      return null;
    }
  ...
  function updateEventDisplay(eventData, showInUserTimezone) {
    ...
    const timeElement = eventData.element.querySelector("#pro-time");
    if (dayElement) {
  ...
  function initProEvents() {
    ...
    const tzCheckbox = document.querySelector("#pro-show-in-my-tz");

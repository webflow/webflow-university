...
(() => {
  ...
  function getNextOccurrence(startDate, frequency, blackoutDates, until) {
    const now2 = DateTime.now();
    let nextOccurrence = startDate;
    const maxIterations = 365;
    let iterations = 0;
    if (startDate > now2 && !isBlackoutDate(startDate, blackoutDates)) {
      return startDate;
    }
  ...
  // src/pro/index.ts
  function parseEventData(element) {
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      if (untilStr && untilStr.trim()) {
        until = parseDate(untilStr, timezone) || void 0;
      }
      recurrences.push({
        startDate,
        frequency,
  ...
  function findNextOccurrence(eventData) {
  ...
  function updateEventDisplay(eventData, showInUserTimezone) {
    ...
    const dayElement = eventData.element.querySelector("#pro-day");

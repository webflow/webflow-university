...
(() => {
  ...
  function getNextOccurrences(startDate, frequency, blackoutDates, count, until) {
    const occurrences = [];
    ...
    while (currentDate <= now2 && iterations < maxIterations) {
  ...
  function parseBlackoutDates(blackoutDateString, timezone) {
    const blackoutDates = [];
    ...
    dateStrings.forEach((dateStr, index) => {
      ...
      if (!date.isValid) {
        ...
        if (parsedDate) {
          date = parsedDate;
          console.error(
            `[Blackout Dates] Date "${dateStr}" was parsed but uses non-standard format. Please use MM/DD/YYYY format (e.g., "10/21/2025" or "1/5/2025")`
          );
        }
      }
      if (date && date.isValid) {
        ...
        console.error(
          `[Blackout Dates] Failed to parse date "${dateStr}" at position ${index + 1}. Expected format: MM/DD/YYYY (e.g., "10/21/2025" or "1/5/2025"). Full string: "${blackoutDateString}"`
        );
      }
    });
    return blackoutDates;
  ...
  function parseSessionData(element) {
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      if (endDateStr && endDateStr.trim()) {
        until = parseDate(endDateStr, timezone) || void 0;
      }
      recurrences.push({
        startDate,
        frequency,
  ...
  function updateRecurrenceUI(index, recurrence, blackoutDates, registrationLink, sessionType) {
    ...
    if (sessionType === "workshop") {
      const tabTextElement = document.getElementById(`datetimes-tab-${index}-workshop-text`) || document.querySelector(`#datetimes-tab-${index}-workshop-text`);
      if (tabTextElement) {
    ...
    occurrences.forEach((occurrence) => {
      ...
      if (sessionType === "workshop") {
        ...
        const dayOfWeek2 = displayDate.toFormat("EEEE");
        contentDiv.textContent = dayOfWeek2;
      } else {
        ...
        const timezoneAbbr = displayDate.toFormat("ZZZZ");
  ...
  function initTemplatePage() {
    const dataSaver = document.querySelector("#data-saver");
    if (!dataSaver) {
